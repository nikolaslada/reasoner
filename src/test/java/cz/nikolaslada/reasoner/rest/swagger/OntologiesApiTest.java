...
@DisplayName("Unit tests for ontologies")
@ExtendWith(MockitoExtension.class)
public class OntologiesApiTest {

    @InjectMocks
    private OntologiesApi ontologiesApi;

    @DisplayName("Test get ontologies")
    @Test
    void getOntologiesTest() {
        Ontologies response = ontologiesApi.getOntologies();
        ...
        assertEquals(response.getOntologyList().get(1).getOwner().getId(), 1);
        assertEquals(response.getOntologyList().get(1).getOwner().getName(), "Admin");
    }
